...
if isinstance(df, pd.DataFrame):
    ...
    with tabs[0]: # ⚔️ 보스 현황
        ...
        # 🚨 정렬 수정: column_config를 사용하여 좌측 정렬 명시
        st.dataframe(
    ...
    with tabs[1]: # 🛡️ 투력 현황
        ...
        st.dataframe(
            cp_rank[['순위', '문파', '이름', '직업', '전투력_v', '성장_표시']], 
    ...
    with tabs[2]: # 🔥 성장 랭킹
        ...
        st.dataframe(
            growth_rank[['순위', '문파', '이름', '성장_표시', '전투력_v']], 
            use_container_width=True, hide_index=True, height=TABLE_HEIGHT,
            column_config={
                "전투력_v": st.column_config.NumberColumn("전투력", alignment="left")
            }
    ...
    with tabs[6]: # 💰 정산 현황
        ...
        if is_admin:
            edited_df = st.data_editor(money_rank[['순위', '이름', '분배금_v', '정산상태']], column_config={"분배금_v": st.column_config.NumberColumn("분배금", alignment="left"), "정산상태": st.column_config.SelectboxColumn("상태", options=["미정산", "정산완료"])}, disabled=["순위", "이름"], hide_index=True, use_container_width=True, height=700)
            ...
            st.dataframe(
                money_rank[['순위', '문파', '이름', '분배금_v', '상태']], 
                use_container_width=True, hide_index=True, height=700,
                column_config={"분배금_v": st.column_config.NumberColumn("분배금", alignment="left")}
            )
