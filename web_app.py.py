...
if isinstance(df, pd.DataFrame):
    ...
    with tabs[2]: # 🔥 성장 랭킹
        ...
        st.dataframe(growth_rank[['순위', '문파', '이름', '성장_표시', '전투력']], use_container_width=True, hide_index=True, height=TABLE_HEIGHT, column_config=UI_CONFIG)
        ...
        with m_col2:
            if not market_df.empty:
                for idx, row in market_df.iterrows():
                    ...
                    st.markdown(f'<div class="{card_class}"><div class="item-info"><div class="item-name">{row["아이템이름"]}</div><div class="item-price">{row["가격"]}</div><div class="item-seller">판매자 : {row["판매자"]}</div></div><div class="status-area"><div class="status-tag {"status-tag-sold" if is_sold else ""}">{row["상태"]}</div></div></div>', unsafe_allow_html=True)
    ...
    with tabs[6]: # 💰 정산 현황
        ...
        if is_admin:
            edited_df = st.data_editor(money_rank[['순위', '이름', '분배금_표시', '정산상태']], column_config={"정산상태": st.column_config.SelectboxColumn("상태", options=["미정산", "정산완료"]), **UI_CONFIG}, disabled=["순위", "이름"], hide_index=True, use_container_width=True, height=700)
