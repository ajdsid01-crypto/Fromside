...
def load_all_guild_data():
    try:
        ...
        # 🚨 [오류 방지] 실제 시트의 행 번호를 보존 (데이터 업데이트용)
...
if isinstance(df, pd.DataFrame):
    ...
    with tabs[6]: # 💰 정산 현황 (안전한 업데이트 로직)
        ...
        if is_admin:
            edited_df = st.data_editor(money_rank[['순위', '이름', '분배금_v', '정산상태', 'sheet_row']], disabled=['순위', '이름', '분배금_v', 'sheet_row'], hide_index=True, use_container_width=True)
