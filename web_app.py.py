...
class GuildManagerApp:
    ...
    def create_widgets(self):
        ...
        self.tab_rank = ttk.Frame(self.notebook); self.tab_money = ttk.import streamlit as st
import pandas as pd
import gspread
...
def add_medal_logic(df):
    ...
    def medal_icon(rank):
        if rank == 1: return "🥇 1위"import streamlit as st
import pandas as pd
import gspread
from oauth2client.service_account import ServiceAccountCredentials
import re
...
if isinstance(df, pd.DataFrame):
    ...
    with tabs[6]: # 정산 (전투력 > 1 & 누계 > 0 정예 필터링)
        ...
        if is_admin:
            edited_df = st.data_editor(money_rank[['순위', '이름', '분배금_v', '정산상태']], disabled=["순위", "이름", "분배금_v"], hide_index=True, use_container_width=True, height=700)
            if st.button("💾 정산 저장"):
